...
class AttendanceService {
  ...
  static Future<bool> markAttendance(String userId, [String? className]) async {
    ...
    if (AuthService.getCurrentUser() != null) {
      ...
      bool isUserInPremises = await LocationService.isUserInPremises(
          checkLatitude, checkLongitude, checkMeter);
  ...
  static Future<void> getAppSettings() async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      checkLatitude = (querySnapshot.docs.first.data()
          as Map<String, dynamic>)['check_latitude'] as double;
      checkLongitude = (querySnapshot.docs.first.data()
          as Map<String, dynamic>)['check_longitude'] as double;
      checkMeter = (querySnapshot.docs.first.data()
          as Map<String, dynamic>)['check_meter'] as double;
  ...
  static Stream<QuerySnapshot> getTodaysPresentUsers({DateTime? date}) {
    return _firestore
        .collection('attendance')
        .where('date',
            isEqualTo: DateFormat('yyyy-MM-dd').format(date ?? DateTime.now()))
        .snapshots();
  }
